...
$allowShutdown=($config.PSObject.Properties.Name -contains 'allow_shutdown') -and ($config.allow_shutdown -eq $true)
...
while($true) {
    try {
        ...
        $payload=@{hostname=$env:COMPUTERNAME;os=$os.Caption;boot_id=$bootId;uptime=[int](([datetime]::Now-$os.LastBootUpTime).TotalSeconds);ack=$ack;reboot_enabled=$config.allow_reboot;shutdown_enabled=$allowShutdown;session_id=$sessionId}
        ...
        if($reply.PSObject.Properties.Name -contains 'command') {
            ...
            Invoke-ShutdownCommand $command $payload $allowShutdown $sessionId $ackPath ([ref]$ack)
