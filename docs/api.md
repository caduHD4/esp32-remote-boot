...
A fila RAM usa a mesma expiração de 30 s. Agent limita espera/validade do ACK a 15 s; reconexão gera nova sessão. Reboot nativo só escreve BootNext depois do ACK, restaurando o valor anterior se o OS recusar reboot. Mensagens de aplicação limitadas a 12 KiB e sem fragmentação no firmware. Keepalive não transporta comandos. Status inclui `agent_transport` (`websocket`/`http-legacy`). Heartbeat HTTP retorna 409 enquanto o WebSocket está autenticado.
